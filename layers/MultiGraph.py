...
class MultiGraph(nn.Module):
    ...
    def __init__(self, dim, d_model, n_heads, seq_len, pred_len, dropout=0.1):
        ...
        self.projection = nn.Linear(dim, n_heads * d_model)
        self.out_projection = nn.Linear(n_heads * d_model, dim)
        self.simple_linear = simple_linear(input_dim=self.seq_len, output_dim=self.pred_len)
        self.dropout = nn.Dropout(dropout)
        self.decompose = series_decomp(self.seq_len // 2 + 1 + self.seq_len // 4)
    ...
    def forward(self, input):
        ...
        att = torch.einsum("bsl,bhl->bsh", multi_features, multi_features)
        ...
        for i in range(H):
            ...
            for j in range(H):
                temp = temp + att[:, i, j].view(B, 1, 1) * multi_features[:, j, :].view(B, 1, -1)
...
class TimeDimGraphBlock(nn.Module):
    ...
    def __init__(self, input_length, dim, dropout, layer_num, block_size=24, cover_size=12):
        ...
        for l in range(layer_num):
            ...
            self.BlockGenerateGraph_module.append(BlockGenerateGraph(block_size, dim, dropout))
