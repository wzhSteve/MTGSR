...
class differential_learning_layer(nn.Module):
    ...
    def forward(self, l_input, r_input):
        ...
        differential_coef = differential_coef.unsqueeze(-1).repeat(1, 1, l_input.shape[-1])
        differential_feature = l_input * differential_coef
        layer_output = self.layer(r_input + differential_feature)
        return layer_output

class differential_learning(nn.Module):
    def __init__(self, input_dim, output_dim, feature_dim=7, dropout=0.01, activation='gelu'):
        """
        :param step: 步长 即每个子序列的长度
        :param attn_layer: 注意力模块初始化
        """
        super(differential_learning, self).__init__()
        ...
        while(count < self.layer):
            ...
            count = count + 1
            self.linear_out_list.append(nn.Linear(output_dim, output_dim, bias=False))
